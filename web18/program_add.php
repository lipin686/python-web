	...
	<h4><center>
		<div>
			...
			<div class="tab-pane well" id="d1">
				...
				<form action="program_save.php" method="post" class="form form-horizontal" enctype="multipart/form-data">
					<p>名稱：<input type="text" name="name" required></p>
					...
					foreach ($faces as $k1 => $face) {
						...
						foreach ($messages as $k2 => $message) {
							...
							<div class="alert alert-info">
								...
								<a href='message_see.php?id=<?=$message['id']?>&type1=1'><?=$face['name']?>:<?=$message['name']?> <br/>評分:<?=$c['s']?></a>
							</div>
							<?php
						}
					}
